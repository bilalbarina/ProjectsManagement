        ...
        <div class="max-w-2xl mx-auto">
            <nav aria-label="Breadcrumb">
                <ol role="list" class="flex items-center gap-1 text-sm text-gray-600">
                    ...
                    <li>
                        <a href="{{ route('project.view', ['project' => $project]) }}" class="block transition hover:text-gray-700"> {{ $project->title }} </a>
                    </li>
                </ol>
            </nav>
            ...
            <div class="mt-4 relative overflow-x-auto sm:rounded-lg border py-4 px-6 flex flex-row justify-between">
                ...
                <div>
                    <span class="text-sm font-semibold"> Assigné: {{ $project->students->count() }} </span>
                </div>
            </div>
        </div>
        <div class="max-w-2xl mx-auto mt-6">
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg border">
                    ...
                    <div>
                        ...
                        <div class="relative mt-1">
                            ...
                            <input type="text" id="search-input"
                                ...
                                placeholder="Recherche apprenents">
                        </div>

                    </div>
                    <div>
                        <a href="{{ route('assignment.all', ['project' => $project]) }}"
                ...
                <div class="overflow-x-auto px-4 my-6" id="students">
                    @component('components.student.assignment-table',
                        [
                            'project' => $project,
                            'students' => $students
                        ])
...
@section('scripts')
    <script>
        ...
        $('#search-input').on('input', function() {
            ...
            $.ajax({
                url: '{{ route('assignment.searchStudents', ['project' => $project]) }}',
                cache: false,
                data: {
                    'name': this.value
                },
