    ...
    <div class="container py-12">
        ...
        <div class="max-w-2xl mx-auto">
            <nav aria-label="Breadcrumb">
                <ol role="list" class="flex items-center gap-1 text-sm text-gray-600">
                    ...
                    <li>
                        <a href="{{ route('project.index') }}" class="block transition hover:text-gray-700"> Projects
                        </a>
                    </li>
                    ...
                    <li>
                        <a href="{{ url()->current() }}" class="block transition hover:text-gray-700"> {{ $project->title }} </a>
                    </li>
                </ol>
            </nav>

            <div class="mt-4 relative overflow-x-auto sm:rounded-lg border py-4 px-6 flex flex-row justify-between">
                <div id="edit-area" class="hidden">
                    <input type="text"
                        ...
                        value="{{ $project->title }}" id="edit-title-input">
                ...
                <div>
                    <span class="text-sm font-semibold"> Taches: {{ $project->tasks->count() }} </span>
                </div>
            </div>
        </div>
        <div class="max-w-2xl mx-auto mt-6">
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg border">
                    ...
                    <div>
                        ...
                        <div class="relative mt-1">
                            ...
                            <input type="text" id="search-input"
                                ...
                                placeholder="Recherche taches">
                        </div>

                    </div>
                    <div class="space-x-3">
                        <a href="{{ route('assignment.index', ['project' => $project]) }}"
                            class="py-2 px-4 border border-green-600 text-green-600 rounded-full text-xs font-semibold">
                            Assigner
                        </a>
                        <a href="{{ route('task.create', ['project' => $project]) }}"
                ...
                <div class="overflow-x-auto px-4 my-6" id="tasks">
                    @component('components.task.table',
                        [
                            'tasks' => $project->tasks
                        ])
                    @endcomponent
...
@section('scripts')
    <script>
        ...
        $('#edit-title-input').on('change', function() {
            $.ajax({
                url: '{{ route('project.updateTitle', ['project' => $project]) }}',
                method: 'POST',
                data: {
                    'title': this.value
                },
        ...
        $('#search-input').on('input', function() {
            ...
            $.ajax({
                url: '{{ route('task.search', ['project' => $project]) }}',
                cache: false,
                data: {
                    'title': this.value
                },
